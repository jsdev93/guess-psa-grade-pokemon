...
function findUniqueImage(urls) {
  ...
  // Find the unique key
  const uniqueKey = keys.find(
    (key) => keys.filter((k) => k === key).length === 1
  );
...
(async () => {
  ...
  const { largeImgSrcs, grade, price, certNumber } = await page.evaluate(() => {
    // Certification number extraction from body text
    let certNumber = null;
    const bodyText = document.body.innerText;
    const certMatch = bodyText.match(
      /Cert(?:ification)?(?:\s*Number)?:?\s*#?\s*(\d{5,})/i
    );
    if (certMatch) {
    ...
    return { largeImgSrcs, grade, price, certNumber };
  });
  // Find the highest resolution images for front and back
  const targetImgFront =
    largeImgSrcs.find((src) => src && src.includes("1600")) || null;
  // Find the unique image by /g/.../s and force the filename to s-l1600.webp
  const uniqueImg = findUniqueImage(largeImgSrcs);
  const targetImgBack = uniqueImg
    ? uniqueImg.replace(/\/[^/]*$/, "/s-l1600.webp")
    : null;
  ...
  try {
    if (!targetImgFront || !targetImgBack) {
      ...
      process.stdout.write(
        JSON.stringify({ imgUrlFront: "No Image", imgUrlBack: "No Image" })
      );
      process.exit(0);
    }
    if (!grade) {
    ...
    const jsonOut = JSON.stringify({
      grade,
      imgUrlFront: targetImgFront,
      imgUrlBack: targetImgBack,
      price,
      certNumber: certNumber || "Null Cert Number",
    });
